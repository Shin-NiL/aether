...
class RunScript {
	...
	private static function build (path:String = "", targets:Array<String> = null, flags:Map <String, String> = null, defines:Array<String> = null):Void {
		...
		for (target in targets) {
			...
			if (target == "tools") {
				...
				var extendedToolsDirectory = PathHelper.getHaxelib (new Haxelib("aether-extended"), false);
				...
				if (extendedToolsDirectory != null && extendedToolsDirectory != "") {
					...
					runCommand (toolsDirectory, "haxe", [ "tools.hxml" ]);
		...
		switch (target) {
			...
			case "android":
				...
				if (!flags.exists ("debug")) {
					...
					runCommand (path, "haxelib", [ "run", buildLib, buildFile, "-Dandroid", "-DPLATFORM=android-9" ].concat (defines));
					synchronizeNDLL ("Android/liblime.so");
					runCommand (path, "haxelib", [ "run", buildLib, buildFile, "-Dandroid", "-DHXCPP_ARMV7", "-DHXCPP_ARM7", "-DPLATFORM=android-9" ].concat (defines));
				...
				if (!flags.exists ("release")) {
					...
					runCommand (path, "haxelib", [ "run", buildLib, buildFile, "-Dandroid", "-DPLATFORM=android-9", "-Ddebug", "-Dfulldebug" ].concat (defines));
					synchronizeNDLL ("Android/liblime-debug.so");
					runCommand (path, "haxelib", [ "run", buildLib, buildFile, "-Dandroid", "-DHXCPP_ARMV7", "-DHXCPP_ARM7", "-DPLATFORM=android-9", "-Ddebug", "-Dfulldebug" ].concat (defines));
			...
			case "wiiu":
				...
				if (!flags.exists ("debug")) {
					...
					runCommand (path, "haxelib", [ "run", buildLib, buildFile, "-Dnintendo", "-Dwiiu", "-Dtoolchain=wiiu", "-I" + PathHelper.getHaxelib (new Haxelib ("lime-wiiu")) ].concat (defines));
					
				}
				
				if (!flags.exists ("release")) {
					
					runCommand (path, "haxelib", [ "run", buildLib, buildFile, "-Dnintendo", "-Dwiiu", "-Ddebug", "-Dfulldebug", "-Dtoolchain=wiiu", "-I" + PathHelper.getHaxelib (new Haxelib ("lime-wiiu")) ].concat (defines));
	...
	private static function getVersion (library:String = "aether", haxelibFormat:Bool = false):String {
		
		//var libraryPath = toolsDirectory;
